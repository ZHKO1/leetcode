def fourSum(nums, target):
    ...
    def threeSum(index,target):
        ...
        while (index < len(nums) - 2):
            ...
            while ((left < right) & (left >= 0) & (right <= len(nums) - 1)):
                if(3 * nums[right] < target):
                    right = right - 1
                    continue
                sumTmp = None
                if(left not in dit):
                    dit[left] = {}
                    if(right not in dit[left]):
                        sumTmp = nums[left] + nums[right]
                        dit[left][right] = sumTmp
                else:
                    if (right in dit[left]):
                        sumTmp = dit[left][right]
                    else:
                        sumTmp = nums[left] + nums[right]
                        dit[left][right] = sumTmp
                if (sumTmp == target - first):
                    result.append([first, nums[left], nums[right]])
                    left = left + 1
                    right = right - 1
                    while ((nums[left] == nums[left - 1]) & (left < len(nums) - 1)):
                        left = left + 1
                    while ((nums[right] == nums[right + 1]) & (right > index)):
                        right = right - 1
                elif (sumTmp < target - first):
                    left = left + 1
                else:
                    right = right - 1
            index = index + 1
            tmp = nums[index]
            while ((tmp == first) & (index < len(nums) - 2)):
                index = index + 1
                tmp = nums[index]
            first = tmp
        return result
    ...
    while (index < len(nums) - 3):
        if(first * 4 > target):
            break;
        threeSumResult = threeSum(index + 1, target - first)
        for item in threeSumResult:
            result.append([first] + item)
        index = index + 1
        tmp = nums[index]
        while ((tmp == first) & (index < len(nums) - 3)):
            index = index + 1
            tmp = nums[index]
        first = tmp
    return result
